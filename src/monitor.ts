...
export async function runMonitor(id: string) {
  ...
  if (runningMonitors.has(id)) throw new Error("Monitor is already running.");
  runningMonitors.add(id);
  try {
    ...
    mutate(state => {
      ...
      saved.seenIds = [...new Set([...saved.seenIds, ...result.entities.map(x => x.id)])].slice(-1000);
    });
    ...
    runningMonitors.delete(id);
  }
}

export function deleteMonitor(id: string) {
  return mutate(state => {
    const deleted = id in state.monitors;
    if (deleted) delete state.monitors[id];
    return { deleted, id };
  });
}
